...
**权限说明**：subagent 可以**读取**文件系统（用于去重检测、线索匹配等），但**不写入**任何文件。所有写入操作由主 agent 执行。
...
5. **在 Stage 4 预览中标注警告**：
   ```
   ⚠️ 内容抓取失败（{错误原因}），仅基于URL和你的描述生成预览。
   存档文件不会创建。如需完整存档，请稍后重试。
   ```
...
3. **构建 Inbox 条目预览**（仅生成内容，不写入文件）：
   ...
   **关联线索区块说明**：
   ...
   - 如果未匹配任何线索：`暂未关联到线索`
   - **此区块在用户确认写入后必须同步更新**（见"主 Agent 写入指南"）
...
2. **匹配已有线索（可多个，但标准要高）**：
   ...
   - **定制化洞察（关键）**：对每条匹配的线索，必须生成 `thread_specific_insight`——即该内容**对这条线索的特定贡献**。不同线索收到的洞察描述必须不同。
     ...
     - 该 `thread_specific_insight` 将在 Stage 4 预览中展示，并在写入时追加到对应 thread
   - 格式：`🔗 [[线索名]] — {thread_specific_insight}`
   - 没有匹配的 → `暂无匹配的已有线索`
...
{关联线索列表，格式与 inbox 文件一致：}
{- [[线索名]] — {thread_specific_insight}}
{如未匹配任何线索：暂未关联到线索}

---

[查看完整存档](../../archive/{filename})
--- 预览结束 ---
...
已有线索匹配（含定制化洞察）：
🔗 [[线索A]] — {thread_specific_insight_A}（将写入该线索的洞察描述）
🔗 [[线索B]] — {thread_specific_insight_B}（将写入该线索的洞察描述）
（或：暂无匹配）
...
{如果触发摘要生成}：
📊 线索 [[{线索名}]] 已累积 {N} 条证据，确认后将自动更新认知摘要。

请确认：写入 / 不存 / 调整
```

**格式说明**：
- 「将写入的内容」区块中的关联线索使用 **inbox 文件格式**：`- [[线索名]] — {thread_specific_insight}`（无 emoji），这是实际写入文件的格式
- 「已有线索匹配」展示区使用 **预览展示格式**：`🔗 [[线索名]] — {thread_specific_insight}`（带 emoji），仅用于向用户清晰展示匹配结果
- 纯想法场景（用户随感）：`**作者**` 行和 `[查看完整存档]` 行需整行删除（参见「非链接消息判断」章节）

### 用户回复的意图识别（容错）

用户不一定精确回复"写入""不存""调整"三个关键词。按以下规则识别意图：

| 意图 | 识别为 | 匹配示例 |
|------|--------|---------|
| 确认写入 | `写入` | "好的"、"存吧"、"ok"、"确认"、"可以"、"写入"、"存" |
| 放弃 | `不存` | "不要了"、"算了"、"取消"、"不存"、"跳过" |
| 要求修改 | `调整` | "改一下XX"、"洞察不太对"、"调整"、"修改"、任何包含具体修改意见的回复 |
| 无法识别 | 追问 | 回复"你是想确认写入，还是需要调整？" |
| 新URL/新想法 | 先处理当前 | 回复"上一条还没确认哦。先告诉我上一条是 写入/不存/调整？然后我再处理新内容。" |

### 用户回复"调整"时的处理流程

用户可能对预览内容不满意，回复"调整"并附带修改意见。处理规则：

**可调整的范围**：
1. **核心洞察文本**：用户觉得提炼不准，要求改写 → 按用户意见修改 `core_insight`
2. **线索匹配关系**：用户认为某个匹配不合理（"别关联到这个线索"）或遗漏了（"也关联到 XX 线索"）→ 增删匹配并重新生成对应的 `thread_specific_insight`
3. **延伸思考**：用户补充或删改 → 直接修改
4. **新线索建议**：用户同意/拒绝/修改新线索名 → 按用户意见调整

**调整后的流程**：
1. 根据用户意见修改对应内容
2. **重新输出完整预览**（格式不变，修改处加 `✏️` 标记）
3. 再次请用户确认：`写入 / 不存 / 继续调整`
4. 不限制调整轮数，直到用户回复"写入"或"不存"
...
**主 agent 收到后转发给用户。用户确认"写入"后，主 agent 按顺序执行：**
1. 写入 inbox 文件（Stage 1 的内容，**直接包含关联线索区块**——基于 Stage 2 的匹配结果填充）
2. 写入存档文件（Stage 3 的内容，如有）
3. 如用户同意线索关联，追加到对应 thread 文件（使用 `thread_specific_insight`，而非全局 `core_insight`）
4. 如触发摘要生成条件，生成或更新对应线索的 `.summary.md` 文件

**注意**：inbox 文件在首次写入时就包含完整的关联线索区块（因为匹配结果在 Stage 2 已确定）。"Inbox 反向索引回写"操作**仅用于**去重选项 b)追加新关联、或用户通过"调整"新增线索匹配等**后续更新**场景。
...
**Stage 4 预览中的对应处理**：纯想法场景的 Stage 4 预览模板同样遵循上述规则——`**作者**` 行删除，`[查看完整存档]` 行删除，`计划写入` 中不包含存档路径行。
...
4. 展示预览：只包含线索追加部分（不创建新 inbox）
5. 用户确认后：
   - 追加到对应 Thread 文件
   - **回写已有 inbox 文件**的「关联线索」区块（追加新关联）
   - 来源链接指向已有的 inbox 文件
...
### 新线索文件创建（仅当用户同意创建新线索时）

**前提**：Stage 2 建议了新线索 `🆕 [[线索名]]`，且用户在确认时明确同意创建。
...
**检查时机**：在 Stage 2 线索匹配完成后、Stage 4 输出预览前检查。读取匹配到的线索文件，统计现有条目数。如果写入后将达到触发阈值（5、10、15...），在 Stage 4 预览末尾加上摘要提示。实际的摘要生成在用户确认写入后、由主 agent 执行。
...
**在 Stage 4 预览中的体现**：如果本次写入触发了摘要生成/更新，在预览末尾提示：
```
📊 线索 [[{线索名}]] 已累积 {N} 条证据，将自动更新认知摘要。
```
